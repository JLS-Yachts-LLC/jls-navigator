    ...
    <div className="flex h-full flex-col">
      ...
      <header className="flex items-center justify-between border-b border-border bg-card/40 px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Sort (works in every view — list columns still sort on click) */}
          <select
            title="Sort vessels"
            value={sortKey ? `${sortKey}:${sortDir}` : "activity"}
            onChange={(e) => {
              const v = e.target.value;
              if (v === "activity") { setSortKey(null); return; }
              const [k, d] = v.split(":");
              setSortKey(k as YachtColumnKey);
              setSortDir((d as SortDir) ?? "asc");
            }}
            className="h-8 rounded-md border border-border bg-card px-2 text-xs text-foreground/80 outline-none focus:border-primary/50"
          >
            <option value="vessel_name:asc">Name A–Z</option>
            <option value="vessel_name:desc">Name Z–A</option>
            <option value="activity">Recently active</option>
            <option value="status:asc">Status</option>
            <option value="flag:asc">Flag</option>
            <option value="vessel_type:asc">Type</option>
            <option value="eta:asc">ETA</option>
            <option value="etd:asc">ETD</option>
          </select>
