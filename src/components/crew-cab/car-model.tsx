/**
 * Parametric 3D vehicle for the Maintenance screen.
 *
 * Built entirely from primitives (no downloaded model), so it works offline,
 * loads instantly, and every mesh can be a NAMED PANEL: tap the bonnet and the
 * app knows it's the bonnet. `bodySpec(type)` reshapes the same component into
 * a coupe, sedan, estate, pickup or van — recognisable silhouettes, not CAD.
 *
...
export const DAMAGE_KEY: Record<DamageKind, { code: string; label: string; color: string }> = {
  ...
  cracked: { code: "C",  label: "Cracked", color: "#fb7185" },
  chipped: { code: "CH", label: "Chipped", color: "#38bdf8" },
  dented:  { code: "D",  label: "Dented",  color: "#f97316" },
  holed:   { code: "H",  label: "Holed",   color: "#a855f7" },
  missing: { code: "M",  label: "Missing", color: "#94a3b8" },
  torn:    { code: "T",  label: "Torn",    color: "#22c55e" },
...
type Role = "paint" | "glass" | "dark" | "trim";
type Part =
