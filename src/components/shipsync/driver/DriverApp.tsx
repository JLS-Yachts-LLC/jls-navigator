...
import { googleMapsDirectionsUrl, type ShipSyncDriver, type ShipSyncDeliveryNote, type ShipSyncPackage, type PackageStatus } from "@/lib/shipsync/model";
import { resolveDriver, listActiveDrivers, loadDriverRuns, scanOntoVan, deliverPackage, type DriverRuns } from "@/lib/shipsync/driver-data";
...
export function DriverApp() {
  ...
  const [runs, setRuns] = useState<DriverRuns>({ notes: [], packages: [] });
  const [loading, setLoading] = useState(true);
  const [online, setOnline] = useState(typeof navigator === "undefined" ? true : navigator.onLine);
  const [queued, setQueued] = useState(0);
  const [view, setView] = useState<"runs" | "note">("runs");
  ...
  const note = runs.notes.find((n) => n.id === noteId) ?? null;
  const notePkgs = runs.packages.filter((p) => p.delivery_note_id === noteId);
  const routeUrl = googleMapsDirectionsUrl(runs.notes.map((n) => ({ address: n.destination_address, lat: n.destination_lat, lng: n.destination_lng })));
    ...
    <div className="mx-auto max-w-xl pb-24">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-4 py-2.5 backdrop-blur">
        ...
        <div className="flex-1">
          ...
          <div className="text-[11px] text-muted-foreground">{runs.notes.length} run{runs.notes.length === 1 ? "" : "s"} today</div>
        </div>
      ...
      {view === "runs" ? (
        <div className="px-4 py-4">
          {routeUrl && runs.notes.length > 0 && (
            <a href={routeUrl} target="_blank" rel="noopener noreferrer" className="mb-3 flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground">
              <MapPin className="h-4 w-4" /> Navigate full route ({runs.notes.length} stops)
            </a>
          )}
          {runs.notes.length === 0 ? (
            <div className="mt-12 text-center text-sm text-muted-foreground">No deliveries assigned to you right now.</div>
          ) : runs.notes.map((n) => {
            const count = runs.packages.filter((p) => p.delivery_note_id === n.id).length;
            const done = runs.packages.filter((p) => p.delivery_note_id === n.id && p.driver_scanned).length;
            const navUrl = googleMapsDirectionsUrl([{ address: n.destination_address, lat: n.destination_lat, lng: n.destination_lng }]);
            return (
              <div key={n.id} className="mb-2.5 rounded-xl border border-border bg-card p-4">
                <button className="w-full text-left" onClick={() => { setNoteId(n.id); setView("note"); }}>
                  <div className="flex items-center justify-between">
                    <span className="font-display text-base font-bold">{n.boat_name ?? "—"}</span>
                    <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-semibold">DN-{n.number}</span>
                  </div>
                  <div className="mt-0.5 text-[12px] text-muted-foreground">{done}/{count} loaded · {n.destination_address || "no address set"}</div>
                </button>
                {navUrl && (
        ...
        <NoteDetail
          note={note} packages={notePkgs} online={online}
          onBack={() => setView("runs")}
...
function NoteDetail({ note, packages, online, onBack, onChanged }: {
  note: ShipSyncDeliveryNote; packages: ShipSyncPackage[]; online: boolean;
  onBack: () => void; onChanged: () => void;
}) {
  const [busyId, setBusyId] = useState<string | null>(null);
  const [deliverPkg, setDeliverPkg] = useState<ShipSyncPackage | null>(null);
  const navUrl = googleMapsDirectionsUrl([{ address: note.destination_address, lat: note.destination_lat, lng: note.destination_lng }]);
  ...
  return (
    <div className="px-4 py-3">
      <button onClick={onBack} className="mb-2 inline-flex items-center gap-1 text-sm text-muted-foreground"><ChevronLeft className="h-4 w-4" /> Runs</button>
      <div className="flex items-center justify-between">
        <h2 className="font-display text-lg font-bold">{note.boat_name} · DN-{note.number}</h2>
      </div>
      {navUrl && (
        <a href={navUrl} target="_blank" rel="noopener noreferrer" className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground">
          <MapPin className="h-4 w-4" /> Navigate to {note.boat_name}
        </a>
      )}
        ...
        {packages.map((p) => (
          <div key={p.id} className="rounded-xl border border-border bg-card p-3">
            ...
            <div className="mt-2 flex gap-2">
              {!p.driver_scanned && (
                <Button size="sm" variant="outline" className="h-9 flex-1 gap-1.5" onClick={() => scan(p)} disabled={busyId === p.id}>
                  {busyId === p.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <ScanLine className="h-4 w-4" />} Load
                </Button>
              )}
              <Button size="sm" className="h-9 flex-1 gap-1.5" onClick={() => setDeliverPkg(p)}><CheckCircle2 className="h-4 w-4" /> Deliver</Button>
            </div>
          </div>
        ))}
        {packages.length === 0 && <div className="py-8 text-center text-sm text-muted-foreground">No packages on this note.</div>}
      </div>

      {deliverPkg && (
        <DeliverDialog pkg={deliverPkg} online={online} onClose={() => setDeliverPkg(null)} onDone={() => { setDeliverPkg(null); onChanged(); }} />
      )}
    </div>
  );
}
...
function DeliverDialog({ pkg, online, onClose, onDone }: {
  pkg: ShipSyncPackage; online: boolean; onClose: () => void; onDone: () => void;
}) {
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success(online ? "Delivered" : "Saved — will sync when back online");
      onDone();
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Proof of delivery — {pkg.barcode}</DialogTitle></DialogHeader>
        <div className="grid gap-3 py-1">
          ...
          <div className="space-y-1.5"><Label className="text-xs">Signature</Label><SignaturePad ref={sigRef} /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={busy}>Cancel</Button>
          <Button onClick={submit} disabled={busy} className="gap-1.5">{busy && <Loader2 className="h-4 w-4 animate-spin" />} Confirm</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
