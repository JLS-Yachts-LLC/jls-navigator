...
import { Users, Building2, Boxes, Plus, Pencil, Trash2, Search, FileText } from "lucide-react";
import {
...
import {
  ...
  type WarehouseClientItem, type WarehouseInternalItem, type WarehousePackageContent, type ManualStatus, type WarehouseDoc,
} from "@/lib/warehouse/data";
import type { WarehouseData } from "@/components/shipsync/ShipSyncWarehouse";

const MANUAL_STATUSES: ManualStatus[] = ["Stored", "Checked Out", "Returned", "Disposed", "Completed"];
...
export function InventoryList({ data, reload }: { data: WarehouseData; reload: () => Promise<void> }) {
  ...
  const [newInternalKind, setNewInternalKind] = useState<"documents" | "assets">("documents");
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {category === "internal" && (
          <Button size="sm" className="gap-1.5" onClick={() => { setNewInternalKind("documents"); setEditingInternal(null); }}><Plus className="h-3.5 w-3.5" /> Add internal item</Button>
        )}
      </div>
            ...
            <>
              {!editingInternal && (
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <div className="flex gap-1 rounded-lg border border-border bg-card/50 p-1 w-fit">
                    <button type="button" onClick={() => setNewInternalKind("documents")}
                      className={cn("rounded-md px-2.5 py-1 text-[12px] font-medium transition-all",
                        newInternalKind === "documents" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                      Document
                    </button>
                    <button type="button" onClick={() => setNewInternalKind("assets")}
                      className={cn("rounded-md px-2.5 py-1 text-[12px] font-medium transition-all",
                        newInternalKind === "assets" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                      Asset
                    </button>
                  </div>
                </div>
              )}
              <InternalItemForm kind={editingInternal?.kind ?? newInternalKind} editing={editingInternal} existingContents={existingContentsForInternalEdit}
...
function FilesCell({ docs }: { docs: WarehouseDoc[] | null | undefined }) {
  if (!docs || docs.length === 0) return <span className="text-muted-foreground">—</span>;
  return (
    <div className="flex max-w-[130px] flex-wrap items-center gap-1">
      {docs.map((d, i) => (
        <SignedAnchor key={i} stored={d.url} title={d.name}
          className="inline-flex max-w-[60px] items-center gap-0.5 truncate rounded border border-border bg-muted/30 px-1 py-0.5 text-[10px] text-primary hover:bg-primary/5">
          <FileText className="h-2.5 w-2.5 shrink-0" /> <span className="truncate">{d.name}</span>
        </SignedAnchor>
      ))}
    </div>
  );
...
/** Derived-status pill plus a Select to change the underlying manual
 *  status — Client/Internal only ever showed the pill read-only, so
 *  changing status meant opening the full edit dialog. */
function EditableStatusCell({ dateField, status, onChange }: { dateField: string | null; status: ManualStatus; onChange: (status: ManualStatus) => void }) {
  const derived = deriveStatus(dateField, status);
  return (
    <div className="flex flex-col items-start gap-1" onClick={(e) => e.stopPropagation()}>
      ...
      <Select value={status} onValueChange={(v) => onChange(v as ManualStatus)}>
...
function ClientTable({ rows, selectedRef, onSelect, onEdit, onDelete, reload }: {
  rows: WarehouseClientItem[]; selectedRef: string | null; onSelect: (ref: string) => void;
  onEdit: (item: WarehouseClientItem) => void; onDelete: (item: WarehouseClientItem) => void; reload: () => Promise<void>;
}) {
  ...
  async function changeStatus(id: string, status: ManualStatus) {
    try { await clientItemCrud.patch(id, { status }); await reload(); }
    ...
    <table className="w-full text-[12.5px]">
      <thead className="sticky top-0 z-10 bg-card shadow-[inset_0_-1px_0_0_var(--border)]">
        <tr>
          ...
          <Th>L×W×H (cm)</Th><Th>Weight</Th><Th>CBM</Th><Th>Charges</Th><Th>Date Stored</Th><Th>Due Date</Th>
          <Th>Location</Th><Th>Invoice No.</Th><Th>Status</Th>
          <Th>Checked-Out Date</Th><Th>Checked Out To</Th><Th>Actual Return Date</Th>
          <Th>Remarks</Th><Th>Files</Th><Th>Image</Th><Th>{""}</Th>
        </tr>
      </thead>
      <tbody className="divide-y divide-border/40">
        {rows.map((r) => (
          <tr key={r.id} onClick={() => onSelect(r.ref_no)}
            ...
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.charges != null ? `AED ${r.charges}` : "—"}</td>
            ...
            <td className="px-3 py-2"><EditableStatusCell dateField={r.due_date} status={r.status} onChange={(s) => void changeStatus(r.id, s)} /></td>
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.checked_out_date ?? "—"}</td>
            <td className="px-3 py-2 text-muted-foreground">{r.checked_out_to ?? "—"}</td>
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.actual_return_date ?? "—"}</td>
            <td className="px-3 py-2 max-w-[160px] truncate text-muted-foreground">{r.remarks || "—"}</td>
            <td className="px-3 py-2" onClick={(e) => e.stopPropagation()}><FilesCell docs={r.documents} /></td>
...
function InternalTable({ rows, selectedRef, onSelect, onEdit, onDelete, reload }: {
  rows: WarehouseInternalItem[]; selectedRef: string | null; onSelect: (ref: string) => void;
  onEdit: (item: WarehouseInternalItem) => void; onDelete: (item: WarehouseInternalItem) => void; reload: () => Promise<void>;
}) {
  ...
  async function changeStatus(id: string, status: ManualStatus) {
    try { await internalItemCrud.patch(id, { status }); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Update failed"); }
  }
  return (
    <table className="w-full text-[12.5px]">
      <thead className="sticky top-0 z-10 bg-card shadow-[inset_0_-1px_0_0_var(--border)]">
        <tr>
          <Th>Ref No.</Th><Th>Department</Th><Th>Package Type</Th><Th>Description</Th>
      ...
      <tbody className="divide-y divide-border/40">
        {rows.map((r) => (
          <tr key={r.id} onClick={() => onSelect(r.ref_no)}
            ...
            <td className="px-3 py-2 capitalize text-muted-foreground">{r.kind}</td>
            ...
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.date_stored ?? "—"}</td>
            <td className="px-3 py-2 font-mono">{locationCode(r)}</td>
            <td className="px-3 py-2"><EditableStatusCell dateField={r.destruction_date} status={r.status} onChange={(s) => void changeStatus(r.id, s)} /></td>
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.checked_out_date ?? "—"}</td>
            <td className="px-3 py-2 text-muted-foreground">{r.checked_out_to ?? "—"}</td>
            <td className="px-3 py-2 tabular-nums text-muted-foreground whitespace-nowrap">{r.actual_return_date ?? "—"}</td>
            <td className="px-3 py-2 max-w-[160px] truncate text-muted-foreground">{r.remarks || "—"}</td>
...
function PackageContentTable({ rows, reload }: { rows: WarehousePackageContent[]; reload: () => Promise<void> }) {
  ...
  async function updateStatus(id: string, status: ManualStatus) {
                ...
                <tr key={c.id} className="hover:bg-accent/10">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex flex-col items-start gap-1">
                      <StatusPill label={derived} style={DISPLAY_STATUS_STYLE[derived]} />
                      <Select value={c.status} onValueChange={(v) => updateStatus(c.id, v as ManualStatus)}>
