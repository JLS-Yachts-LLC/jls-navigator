/**
 * ShipSync EDAS tab ↔ Monday.com import.
 *
 * One-way, read-only mirror of the Monday "EDAS 2026" board (id 5089054389)
 * into shipsync_packages (local_import = 'EDAS') — Monday is the source of
 * truth, this never writes back. Modelled directly on
 * monday-export-board.server.ts (same hardcoded-board-id shape, same
 * column_values → byTitle/pick approach), kept in its own file since EDAS
 * has an unrelated, much simpler column layout: one flat group ("EDAS 2026"),
 * and just AWB / CLIENT / Files / Date per row — no multi-stage status
 * workflow like Import/Export/Local have.
 *
 * The board also carries a "Subitems" column (linked sub-board) and a
 * "Mirror" column (a status mirrored from a linked item on a companion
 * board, id 5089749617, via a "link to EDAS 2026" board_relation column) —
 * every sampled row had both blank, so neither is mapped onto a first-class
 * field here. Their raw text (if Monday ever populates them) still comes
 * through generically via extra.monday, same as any other unmapped column,
 * and shows via the board's "extra Monday columns" fallback.
 *
 ...
 * fixed (this board doesn't change), so like Export it's hardcoded below
 * rather than a separate config key.
 */
...
const EDAS_BOARD_ID = '5089054389'
...
// it can never contend with the Local/Import/Export board syncs' locks.
...
async function acquireSyncLock(): Promise<boolean> {
  ...
  const lockedAt = cfg.edas_board_sync_started_at ? new Date(cfg.edas_board_sync_started_at).getTime() : 0
  ...
  await db().from('integration_settings')
    .update({ config: { ...cfg, edas_board_sync_started_at: new Date().toISOString() } })
    .eq('integration_name', 'monday')
  return true
}
...
async function releaseSyncLock(): Promise<void> {
  const { data: row } = await db()
    ...
    .maybeSingle()
  const cfg = row?.config ?? {}
  delete cfg.edas_board_sync_started_at
  await db().from('integration_settings')
    .update({ config: cfg })
    .eq('integration_name', 'monday')
}
 ...
 *  touches the Local/Import/Export syncs' rows (scoped to local_import = 'EDAS' throughout). */
export async function importMondayEdasBoard(): Promise<MondayEdasBoardResult> {
  const gotLock = await acquireSyncLock()
  if (!gotLock) {
    return { ok: true, synced: 0, errors: 0, pruned: 0, skipped: true, detail: 'Skipped — another ShipSync EDAS Monday sync is already in progress.' }
  }
  try {
...
/** Server function for the "Sync from Monday" button on the ShipSync EDAS board. */
export const syncMondayEdasBoard = createServerFn({ method: 'POST' })
  .handler(async (): Promise<MondayEdasBoardResult> => importMondayEdasBoard())
