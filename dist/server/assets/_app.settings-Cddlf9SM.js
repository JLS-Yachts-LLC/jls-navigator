import { c as createServerRpc } from "./createServerRpc-D4n2ocfV.js";
import { a2 as createServerFn, a3 as supabaseAdmin, a4 as getGraphToken, a5 as resolveSpSite, a6 as saveSpConfigPatch, a7 as syncFromSharePoint, a8 as registerSharePointWebhook, a9 as renewSharePointWebhook, aa as getSpSyncs, ab as saveSpSync, ac as deleteSpSync, ad as syncById } from "./worker-entry-BhSB73Oa.js";
import "node:events";
...
const getUsers = createServerFn({
  ...
  const [{
    ...
    perPage: 1e3
  }), supabaseAdmin.from("user_roles").select("user_id, role"), supabaseAdmin.from("profiles").select("id, display_name, first_name, last_name")]);
...
const doUpdateProfile_createServerFn_handler = createServerRpc({
  id: "7e6df29da9e1b91876d6bf37eafdc21bc147471ce1396ec890a2024405c14c41",
  name: "doUpdateProfile",
  filename: "src/routes/_app.settings.tsx"
}, (opts) => doUpdateProfile.__executeServer(opts));
const doUpdateProfile = createServerFn({
  method: "POST"
}).handler(doUpdateProfile_createServerFn_handler, async (ctx) => {
  const {
    userId,
    firstName,
    lastName
  ...
  const {
    error
  } = existing ? await supabaseAdmin.from("profiles").update(payload).eq("id", userId) : await supabaseAdmin.from("profiles").insert(payload);
  if (error) throw new Error(error.message);
});
...
const doGetSpSyncs_createServerFn_handler = createServerRpc({
  id: "5b4922fc84c47fc65b18897ea783d6a76a41a68ce5e14c32aae3609057c7a2f7",
  name: "doGetSpSyncs",
  filename: "src/routes/_app.settings.tsx"
}, (opts) => doGetSpSyncs.__executeServer(opts));
const doGetSpSyncs = createServerFn({
  method: "GET"
}).handler(doGetSpSyncs_createServerFn_handler, async () => getSpSyncs());
const doSaveSpSync_createServerFn_handler = createServerRpc({
  id: "4aeec71de219dff2fc3ee14a9cda1d5e07b5e5f336005f4c5f28bc850f8dca75",
  name: "doSaveSpSync",
  filename: "src/routes/_app.settings.tsx"
}, (opts) => doSaveSpSync.__executeServer(opts));
const doSaveSpSync = createServerFn({
  method: "POST"
}).handler(doSaveSpSync_createServerFn_handler, async (ctx) => {
  return saveSpSync(ctx.data);
});
const doDeleteSpSync_createServerFn_handler = createServerRpc({
  id: "8a9867d86ae6a5fb8bb44b5d97bb762f1e406f9eeb114fe87c085a22ad773638",
  name: "doDeleteSpSync",
  filename: "src/routes/_app.settings.tsx"
}, (opts) => doDeleteSpSync.__executeServer(opts));
const doDeleteSpSync = createServerFn({
  method: "POST"
}).handler(doDeleteSpSync_createServerFn_handler, async (ctx) => {
  return deleteSpSync(ctx.data.id);
});
const doSyncById_createServerFn_handler = createServerRpc({
  id: "7c2148c13e358b569c9bece2a9fdb5d711b9b3e59062970e4038f5aff7cb93c7",
  name: "doSyncById",
  filename: "src/routes/_app.settings.tsx"
}, (opts) => doSyncById.__executeServer(opts));
const doSyncById = createServerFn({
  method: "POST"
}).handler(doSyncById_createServerFn_handler, async (ctx) => {
...
export {
  ...
  doSaveSpSync_createServerFn_handler,
  doSetRole_createServerFn_handler,
  doSyncById_createServerFn_handler,
  doSyncSharePoint_createServerFn_handler,
  doUpdateProfile_createServerFn_handler,
